...
try:
    from LaneDetector.ufldDetector.utils.dist_utils import get_rank, get_world_size, is_main_process, dist_print, DistSummaryWriter
    from LaneDetector.ufldDetector.utils.config import Config
except:
...
import numpy as np
def merge_config(arguments = None):
    if arguments == None:
        args = get_args().parse_args()
    else:
        args = get_args().parse_args(arguments)  
...
def get_model(cfg):
    try:
        ...
        return importlib.import_module('LaneDetector.model.model_'+cfg.dataset.lower()).get_model(cfg)
